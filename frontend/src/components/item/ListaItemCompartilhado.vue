<template lang="html">
  <div class="lista-items-compartilhados row" v-if="this.$root.credentials">
    <div class="col-md-10 col-md-offset-1 text-left">
      ...
      <tbody>
        <tr v-for="item in items" v-if="item.nome.includes(busca) || item.descricao.includes(busca)">
          <td>{{item.nome}}</td>
          <td>{{item.descricao}}</td>
          <td>{{item.tipo}}</td>
          <td class="item-options">
            <b-icon-pencil @click="edita(item)"></b-icon-pencil>
            <b-icon-eye @click="detalhes(item)"></b-icon-eye>
            <b-icon-trash @click="remove(item)"></b-icon-trash>
          </td>
        </tr>
      </tbody>
      </table>
